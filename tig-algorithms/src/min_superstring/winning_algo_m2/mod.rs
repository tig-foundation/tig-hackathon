...
pub fn solve_challenge(
    ...
    // Build letter counts for all input strings (we only permute; letters are fixed)
    let counts: Vec<[u8; 26]> = challenge.strings.iter().map(|s| counts_of(s)).collect();

    // ----- scoring for an order using sequential feasible overlaps ----------
    // Returns (saved_chars, per-edge overlap vectors) if `keep_edges` is true
    fn score_order(
        order: &[usize],
        counts: &[[u8; 26]],
        keep_edges: bool,
    ...
    // ----- initial orders: multi-start greedy by pairwise intersection ------
    ...
    // Choose up to K seeds with largest total pairwise intersection
    ...
    // A few randomized seeds for diversity
    let extra = (k / 2).max(1);
    for _ in 0..extra {
    ...
    // Pick the best initial order by the sequential score
    let mut best_order = {
    ...
    // ----- randomized 2-opt improvement on the order ------------------------
    // We keep it lightweight and deterministic via rng seeded above.
    ...
    // ----- build final solution from the best order --------------------------
    // 1) Compute exact per-edge overlap vectors under the sequential model
    let (saved_final, Some(edge_vecs)) = score_order(&best_order, &counts, true) else {
        unreachable!();
    };

    // 2) Build prefix/suffix sequences for each string (as Strings)
    let mut prefix_seq: Vec<String> = vec![String::new(); n];
    let mut suffix_seq: Vec<String> = vec![String::new(); n];

    // edge j corresponds to best_order[j] -> best_order[j+1]
    for j in 0..best_order.len().saturating_sub(1) {
        let a = best_order[j];
        let b = best_order[j + 1];
        let x = &edge_vecs[j];

        // Deterministic order inside the overlap: alphabetical with multiplicities
    ...
    // 3) Build permuted strings (prefix + middle + suffix) honoring counts
    ...
    for idx in 0..n {
        ...
        // middle: remaining letters not used by prefix or suffix (alphabetical)
        let mut need = [0u8; 26];
        for c in 0..26 {
        ...
        // (optional) sanity: used should equal original counts
    ...
    // We don't need to store the superstring bytes; its length and indices suffice.
    for j in 1..best_order.len() {
        let curr = best_order[j];
        let ov = prefix_seq[curr].len();
        // current string starts where the overlap with the existing suffix begins
        starts[curr] = super_len.saturating_sub(ov);
        // extend length by the non-overlapped tail of the current permuted string
        super_len += 5 - ov;
    }
