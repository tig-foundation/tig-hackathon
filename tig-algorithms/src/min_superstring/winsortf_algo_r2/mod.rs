...
pub fn solve_challenge(
    ...
    // Generate all unique permutations for each input string
    ...
    // Initial state using greedy algorithm
    ...
    while start_time.elapsed().as_millis() < time_limit_ms {
        ...
        if rng.gen::<f32>() < 0.8 && n > 2 {
            ...
            if all_perms[str_idx_to_change].len() <= 1 {
                continue;
            }

            let new_perm_str = all_perms[str_idx_to_change].choose(&mut rng).unwrap();
            if *new_perm_str == current_perms[str_idx_to_change] {
                continue;
            }
...
fn calculate_overlap(s1: &str, s2: &str) -> usize {
    (1..=4).rev().find(|&k| s1.ends_with(&s2[..k])).unwrap_or(0)
}
